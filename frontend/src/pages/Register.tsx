...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // If user exists (from Login redirect), we only need Name and Phone.
      // If new user, we need everything.
      if (!name || !phone || (!firebaseUser && (!email || !password))) {
      ...
      // 3. Force Sign Out & Redirect to Login
      await signOut(auth);
  ...
  const handleGoogle = async () => {
    try {
      ...
      try {
        ...
        // User doesn't exist, proceed to register in backend with defaults
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <motion.div
        ...
        className="w-full max-w-md glass rounded-xl border border-border p-8"
